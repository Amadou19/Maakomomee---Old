...
class Register_ViewModel @Inject constructor(private val authenticationRepository : AuthenticationRepository): ViewModel() {
    ...
    fun creatUsers(email: String, password: String) {
        ...
        signState.value = Sign_ViewModel_State.Editable(progressBarVisiblity = true)
        ...
        when{
            ...
            password.length !in(7..20)-> signState.value = Sign_ViewModel_State.Editable(passewordErrorTview = R.string.password_error, passewordErrortVisiblity = true)
    ...
    private fun handleAuthResult(authResult: Single<Task<AuthResult>>) {
        compositeDisposable += authResult
            ...
            it.addOnSuccessListener { signState.value = Sign_ViewModel_State.Success(Event(R.string.sucssec_sign)) }
